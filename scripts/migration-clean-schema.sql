-- Clean Schema Migration - Safe to run multiple times
-- This migration safely adds missing tables and fixes constraints
-- It uses DROP IF EXISTS and CREATE IF NOT EXISTS to be idempotent
...
-- Projects table (required by actions schema)
...
-- ============================================================================
-- ORGANIZATIONS (if not already created)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS deployments_platform_idx ON public.deployments(platform);
...
-- ============================================================================
-- FILE SNAPSHOTS (for code change tracking)
-- ============================================================================
...
-- Pages RLS Policies
DROP POLICY IF EXISTS "Users can view pages they have access to" ON public.pages;
...
DROP POLICY IF EXISTS "Users can create file snapshots in their projects" ON public.file_snapshots;
CREATE POLICY "Users can create file snapshots in their projects"
...
DROP POLICY IF EXISTS "Users can update file snapshots in their projects" ON public.file_snapshots;
CREATE POLICY "Users can update file snapshots in their projects"
  ON public.file_snapshots FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = file_snapshots.project_id
...
DROP POLICY IF EXISTS "Users can delete file snapshots from their projects" ON public.file_snapshots;
CREATE POLICY "Users can delete file snapshots from their projects"
  ON public.file_snapshots FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = file_snapshots.project_id
