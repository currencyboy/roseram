-- ============================================================================
-- ROSERAM BUILDER - UNIFIED SCHEMA MIGRATION (IDEMPOTENT)
-- SAFE TO RUN MULTIPLE TIMES - Uses IF NOT EXISTS for all DDL statements
-- ============================================================================
...
-- ============================================================================
-- SECTION 10: ROW LEVEL SECURITY (RLS) SETUP
-- ============================================================================
...
CREATE POLICY "Users can view organization members"
  ON public.organization_members FOR SELECT USING (
    EXISTS (
      ...
      AND (organizations.owner_id = auth.uid() OR EXISTS (
...
-- ============================================================================
-- RLS POLICIES - Code Generations & Chat
-- ============================================================================

DROP POLICY IF EXISTS "Users can view generations for their projects" ON public.code_generations;
CREATE POLICY "Users can view generations for their projects"
  ON public.code_generations FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = code_generations.project_id AND projects.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Users can create generations for their projects" ON public.code_generations;
CREATE POLICY "Users can create generations for their projects"
  ON public.code_generations FOR INSERT WITH CHECK (
...
DROP POLICY IF EXISTS "Users can view chat for their projects" ON public.chat_messages;
...
DROP POLICY IF EXISTS "Users can add chat for their projects" ON public.chat_messages;
...
CREATE POLICY "Users can view revisions for their projects"
  ON public.file_revisions FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = file_revisions.project_id AND projects.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Users can create revisions for their projects" ON public.file_revisions;
CREATE POLICY "Users can create revisions for their projects"
  ON public.file_revisions FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = file_revisions.project_id AND projects.user_id = auth.uid()
    )
...
CREATE POLICY "Users can view file snapshots from their projects"
  ON public.file_snapshots FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = file_snapshots.project_id AND projects.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Users can create snapshots for their projects" ON public.file_snapshots;
CREATE POLICY "Users can create snapshots for their projects"
  ON public.file_snapshots FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = file_snapshots.project_id AND projects.user_id = auth.uid()
    )
...
DROP POLICY IF EXISTS "Users can view action logs for their projects" ON public.action_logs;
CREATE POLICY "Users can view action logs for their projects"
  ON public.action_logs FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = action_logs.project_id AND projects.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Users can create action logs for their projects" ON public.action_logs;
CREATE POLICY "Users can create action logs for their projects"
  ON public.action_logs FOR INSERT WITH CHECK (
    auth.uid() = user_id AND EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = action_logs.project_id AND projects.user_id = auth.uid()
    )
...
DROP POLICY IF EXISTS "Users can create sites in their organizations" ON public.sites;
CREATE POLICY "Users can create sites in their organizations"
  ON public.sites FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.organization_members
      WHERE organization_members.organization_id = sites.organization_id
      AND organization_members.user_id = auth.uid()
...
DROP POLICY IF EXISTS "Users can update sites in their organizations" ON public.sites;
...
CREATE POLICY "Users can view pages in their organizations"
  ON public.pages FOR SELECT USING (
    EXISTS (
      ...
      WHERE sites.id = pages.site_id AND EXISTS (
        SELECT 1 FROM public.organization_members
        WHERE organization_members.organization_id = sites.organization_id
        AND organization_members.user_id = auth.uid()
      )
...
DROP POLICY IF EXISTS "Users can create pages in their sites" ON public.pages;
...
DROP POLICY IF EXISTS "Users can update pages in their sites" ON public.pages;
...
DROP POLICY IF EXISTS "Users can view page versions" ON public.page_versions;
CREATE POLICY "Users can view page versions"
  ON public.page_versions FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.pages
      JOIN public.sites ON pages.site_id = sites.id
      WHERE pages.id = page_versions.page_id AND EXISTS (
        SELECT 1 FROM public.organization_members
        WHERE organization_members.organization_id = sites.organization_id
        AND organization_members.user_id = auth.uid()
      )
...
DROP POLICY IF EXISTS "Users can create page versions" ON public.page_versions;
CREATE POLICY "Users can create page versions"
  ON public.page_versions FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.pages
      JOIN public.sites ON pages.site_id = sites.id
      WHERE pages.id = page_versions.page_id AND EXISTS (
...
DROP POLICY IF EXISTS "Users can view components in their organizations" ON public.components;
...
DROP POLICY IF EXISTS "Users can view sections in their organizations" ON public.sections;
...
CREATE POLICY "Users can view their own AI generations"
  ON public.ai_generations FOR SELECT USING (
    auth.uid() = user_id OR EXISTS (
      ...
      WHERE pages.id = ai_generations.page_id AND organization_members.user_id = auth.uid()
    ) OR EXISTS (
      SELECT 1 FROM public.sites
      JOIN public.organization_members ON sites.organization_id = organization_members.organization_id
      WHERE sites.id = ai_generations.site_id AND organization_members.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Users can create AI generations for their resources" ON public.ai_generations;
CREATE POLICY "Users can create AI generations for their resources"
  ON public.ai_generations FOR INSERT WITH CHECK (
    auth.uid() = user_id AND (
      EXISTS (
        SELECT 1 FROM public.pages
        JOIN public.sites ON pages.site_id = sites.id
        JOIN public.organization_members ON sites.organization_id = organization_members.organization_id
        WHERE pages.id = page_id AND organization_members.user_id = auth.uid()
        AND organization_members.role IN ('owner', 'admin', 'editor')
      ) OR EXISTS (
        SELECT 1 FROM public.sites
        JOIN public.organization_members ON sites.organization_id = organization_members.organization_id
        WHERE sites.id = site_id AND organization_members.user_id = auth.uid()
        AND organization_members.role IN ('owner', 'admin', 'editor')
      )
    )
  );
...
CREATE POLICY "Users can view deployments for their projects"
  ON public.deployments FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.projects
      WHERE projects.id = deployments.project_id AND projects.user_id = auth.uid()
    ) OR EXISTS (
      SELECT 1 FROM public.sites
      JOIN public.organization_members ON sites.organization_id = organization_members.organization_id
      WHERE sites.id = deployments.site_id AND organization_members.user_id = auth.uid()
    )
  );
...
-- ============================================================================
-- RLS POLICIES - Integrations & Keys
-- ============================================================================

DROP POLICY IF EXISTS "Users can view their organization integrations" ON public.integrations;
...
CREATE POLICY "Only admins can manage integrations"
  ON public.integrations FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.organization_members
      WHERE organization_members.organization_id = integrations.organization_id
      AND organization_members.user_id = auth.uid()
...
DROP POLICY IF EXISTS "Users can view their integrations" ON public.user_integrations;
...
-- ============================================================================
-- RLS POLICIES - Billing & Analytics
-- ============================================================================
...
DROP POLICY IF EXISTS "Users can view their usage quotas" ON public.usage_quotas;
CREATE POLICY "Users can view their usage quotas"
  ON public.usage_quotas FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.organization_members
...
CREATE POLICY "Users can view their API usage"
  ON public.api_usage FOR SELECT USING (
    auth.uid() = user_id OR (organization_id IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.organization_members
...
DROP POLICY IF EXISTS "Users can view page analytics" ON public.page_analytics;
CREATE POLICY "Users can view page analytics"
  ON public.page_analytics FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.pages
      JOIN public.sites ON pages.site_id = sites.id
      JOIN public.organization_members ON sites.organization_id = organization_members.organization_id
      WHERE pages.id = page_analytics.page_id AND organization_members.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Users can view error logs for their resources" ON public.error_logs;
CREATE POLICY "Users can view error logs for their resources"
  ON public.error_logs FOR SELECT USING (
    auth.uid() = user_id OR (organization_id IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.organization_members
      ...
      SELECT 1 FROM public.projects
      WHERE projects.id = error_logs.project_id AND projects.user_id = auth.uid()
    ))
  );

DROP POLICY IF EXISTS "Users can view activity logs for their organization" ON public.activity_logs;
...
CREATE POLICY "Users can view page comments"
  ON public.page_comments FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.pages
      JOIN public.sites ON pages.site_id = sites.id
      JOIN public.organization_members ON sites.organization_id = organization_members.organization_id
      WHERE pages.id = page_comments.page_id AND organization_members.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Users can create page comments" ON public.page_comments;
CREATE POLICY "Users can create page comments"
  ON public.page_comments FOR INSERT WITH CHECK (
    auth.uid() = user_id AND EXISTS (
      SELECT 1 FROM public.pages
      JOIN public.sites ON pages.site_id = sites.id
      JOIN public.organization_members ON sites.organization_id = organization_members.organization_id
...
DROP VIEW IF EXISTS public.user_organizations CASCADE;
...
DROP VIEW IF EXISTS public.user_sites CASCADE;
CREATE VIEW public.user_sites AS
SELECT
  u.id as user_id,
  s.id as site_id,
  s.name,
...
DROP VIEW IF EXISTS public.site_stats CASCADE;
CREATE VIEW public.site_stats AS
SELECT
  s.id,
  s.organization_id,
