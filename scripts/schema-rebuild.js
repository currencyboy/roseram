...
async function rebuildSchema() {
  try {
    console.log('╔════════════════════════════════════════════════════════════╗');
    console.log('║   Roseram Builder - Database Schema Rebuild               ║');
    console.log('╚════════════════════════════════════════════════════════════╝\n');

    // Write SQL to temp file
    const tempFile = path.join(__dirname, '..', 'temp_schema.sql');
    fs.writeFileSync(tempFile, SCHEMA_SQL);
    ...
    console.log('To complete the schema rebuild:\n');
    console.log('1. Go to: https://app.supabase.com/project/' + projectRef + '/sql');
    console.log('2. Click "New Query"');
    console.log('3. Copy and paste the content from: ' + tempFile);
    console.log('4. Click "RUN" to execute\n');

    console.log('Or use the Supabase CLI:\n');
    console.log('   supabase db push\n');

    console.log('✅ SQL file created at:', tempFile);
    console.log('\n📌 Alternatively, you can use psql if PostgreSQL is installed:\n');
    console.log('   export PGPASSWORD="' + SUPABASE_SERVICE_ROLE.substring(0, 20) + '..."');
    console.log('   psql -h db.' + projectRef + '.supabase.co -U postgres -d postgres -f ' + tempFile + '\n');
